...
class GatewayRunner:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        try:
            ...
            # next turn continues toward the goal automatically.
            try:
                session_entry = self.session_store.get_or_create_session(source)
                self._post_turn_goal_continuation(
                    session_entry=session_entry,
                    source=source,
                    final_response=_agent_result if isinstance(_agent_result, str) else "",
                )
    ...
    async def _send_update_notification(self) -> bool:
        ...
        try:
            ...
            user_config = load_cli_config()
            if user_config.get("update", {}).get("notify") is False:
                # Clean up pending files without notifying
                pending_path = _hermes_home / ".update_pending.json"
                claimed_path = _hermes_home / ".update_pending.claimed.json"
                output_path = _hermes_home / ".update_output.txt"
                exit_code_path = _hermes_home / ".update_exit_code"
                for p in (pending_path, claimed_path, output_path, exit_code_path):
                    p.unlink(missing_ok=True)
                return True
        except Exception:
            pass
